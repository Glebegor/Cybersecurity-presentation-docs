...
### Wifi
Ochrana Wi-Fi sítě je klíčová pro zabezpečení vašeho internetového připojení a ochranu před neoprávněným přístupem k vašim datům. Jedním z nejdůležitějších kroků je použití silného hesla pro Wi-Fi síť, které je složité a těžko uhádnutelné. Dále je důležité aktivovat šifrování sítě pomocí moderního standardu, jako je WPA2, které zabraňuje odposlechu dat. Skrytí SSID sítě může snížit riziko neoprávněného přístupu, ačkoliv není úplně účinné. Omezení přístupu na základě MAC adres zařízení umožňuje povolení přístupu pouze pro určitá zařízení. Pravidelné aktualizace firmwaru vašeho směrovače a dalších síťových zařízení jsou důležité pro zajištění nejnovějších zabezpečení. Používání VPN při používání veřejných Wi-Fi sítí šifruje veškerý síťový provoz a chrání vaše data před odposlechem.

Retranslační útok, známý také jako "man-in-the-middle" útok, je technika, kterou útočník využívá k interceptaci a manipulaci komunikace mezi dvěma stranami, aniž by bylo možné tuto manipulaci detekovat. Útočník se umísťuje mezi komunikující strany a přenáší data mezi nimi, zatímco si zachovává možnost monitorovat a dokonce i modifikovat přenášená data.
